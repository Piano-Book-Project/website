...
export const playlistRouter = router({
  ...
  next: procedure.input(z.object({ userId: z.number(), currentSongId: z.number() })).query(async ({ input }) => {
  ...
  prev: procedure.input(z.object({ userId: z.number(), currentSongId: z.number() })).query(async ({ input }) => {
    const { userId, currentSongId } = input;
    const playlist = await prisma.playlist.findMany({
      where: { userId },
      orderBy: { playedAt: 'asc' },
