...
export const artistRouter = router({
  ...
  get: procedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const artist = await prisma.artist.findUnique({ where: { id: input.id }, include: { songs: true, playlists: true } });
    if (!artist) throw new TRPCError({ code: 'NOT_FOUND', message: '존재하지 않는 아티스트입니다.' });
    return artist;
  }),
