...
const PricingManagement: React.FC = () => {
    ...
    // 실제 DB에서 요금제 목록 불러오기 (현재는 목업 데이터)
    const { data: pricingPlans = [], isLoading, isError, refetch } = useQuery<PricingPlan[]>({
        queryKey: ['pricing', searchTerm, statusFilter, billingFilter],
        queryFn: async () => {
            ...
            return [
                ...
                },
                {
                    ...
                    currency: 'KRW',
                    billingCycle: 'monthly',
                    features: ['모든 Basic 기능', '50GB 저장공간', '우선 지원', '고급 분석'],
                    isActive: true,
                    isPopular: true,
                    sortOrder: 2,
                    createdAt: '2025-07-12T10:00:00Z',
                ...
                },
                {
                    ...
                    currency: 'KRW',
                    billingCycle: 'monthly',
                    features: ['모든 Pro 기능', '무제한 저장공간', '전담 지원', '맞춤형 솔루션'],
                    isActive: true,
                    isPopular: false,
                    sortOrder: 3,
                    createdAt: '2025-07-12T10:00:00Z',
                ...
                },
                {
                    id: 4,
                    name: 'Lifetime',
                    description: '평생 이용 가능한 원타임 결제',
                    price: 299000,
                    currency: 'KRW',
                    billingCycle: 'lifetime',
                    features: ['모든 기능', '평생 업데이트', '평생 지원'],
                    isActive: true,
                    isPopular: false,
                    sortOrder: 4,
                    createdAt: '2025-07-12T10:00:00Z',
    ...
    const handleBillingFilter = (event: any) => {
    ...
    const getBillingLabel = (billingCycle: string) => {
        switch (billingCycle) {
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, mt: 4, ml: 0, mr: 0 }}>
                <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        총 {filteredPricingPlans.length}개의 요금제가 있습니다
                    </Typography>
                </Box>
                <Button
                    variant="contained"
                        ...
                        {isLoading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={9} align="center">
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        검색 조건을 변경하거나 새로운 요금제를 추가해보세요
                                    </Typography>
                                </TableCell>
                            </TableRow>
                        ) : (
                            filteredPricingPlans.map((plan, index) => (
                                <TableRow key={plan.id} hover sx={{
                                    background: '#141517',
                                    '&:hover': {
                                        background: '#1e2127',
                                    ...
                                    <TableCell sx={{
                                        ...
                                        px: 3,
                                        fontSize: 14
                                    }}>{formatPrice(plan.price, plan.currency)}</TableCell>
                                    <TableCell sx={{
                                        color: '#e0e7ef',
                                        py: 2.5,
                                        px: 3,
                                        fontSize: 14
                                    }}>{getBillingLabel(plan.billingCycle)}</TableCell>
                                    <TableCell sx={{
                                        py: 2.5,
                                        px: 3,
                                        fontSize: 14
                                    ...
                                    <TableCell sx={{
                                        ...
                                        px: 3,
                                        fontSize: 14
                                    }}>{new Date(plan.createdAt).toLocaleDateString()}</TableCell>
                                    <TableCell align="center" sx={{
                                        ...
                                        <IconButton
                                            size="small"
                                            onClick={() => navigate(`/pricing/edit/${plan.id}`)}
                                            sx={{
                                                color: '#60a5fa',
                                                '&:hover': {
                                                    bgcolor: '#1e2127',
